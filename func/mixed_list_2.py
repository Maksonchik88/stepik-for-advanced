mixed_list = ['beside', 48, 'accelerate', 28, 'beware', 'absorb', 'besides', 'berry', 15, 65, 'abate', 'thursday',
              ...
              'abandon', 'accept', 46, 'abide', 'beyond', 19, 'about', 76, 26, 'abound', 12, 95, 'wednesday', 'abundant',
              'abrupt', 'aboard', 50, 89, 'tuesday', 66, 'bestow', 'absent', 76, 46, 'betray', 47, 'able', 11]
nums_lst = list(sorted(filter(lambda st: str(st).isdigit(), mixed_list)))
words_lst = list(sorted(filter(lambda st: str(st).isalpha(), mixed_list)))
print(*nums_lst + words_lst)